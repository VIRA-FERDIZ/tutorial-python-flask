...
def quotes():
	quotes_file = json.loads(open('./server/quotes.json').read())
...
@app.route('/api/bucin', methods=['GET','POST'])
def bucin():
	bucin_file = json.loads(open('./server/bucin.json').read())
	result = random.choice(bucin_file)
	print(result)
	return {
		'status': 200,
		'author': result['author'],
...
@app.route('/api/dewasa', methods=['GET','POST'])
def dewasa():
	dewasa_file = json.loads(open('./server/18+.json').read())
	result = random.choice(dewasa_file)
	print(result)
	return {
		'status': 200,
		'teks': result['teks'],
...
@app.route('/api/anime/hentai', methods=['GET','POST'])
def hentai():
	try:
		neko = get('https://nekos.life/api/v2/img/hentai').json()
		nimek = neko['url']
		return {
			'status': 200,
			'result': nimek
